...
var SudokuSolver = function (testable) 
{
  ...
  function solve(boardString) 
  {
    ...
    return recursiveSolve(boardString); // returning the string of solved sudoku
  }

  function solveAndPrint(boardString) {
    var solvedBoard = solve(boardString);
    console.log(toString(solvedBoard.split(""))); // prints the whole sudoku
    return solvedBoard;
  }
  ...
  function recursiveSolve(boardString) {
    ...
    {
      boardArray[nextUnsolvedCellIndex] = possibilities[i]; // assigining the chosen values for the cells at the correct index
  ...
  function getNextCellAndPossibilities(boardArray) { // here the function returns the possible choices for the empty cells(with index).
    for(var i = 0; i < boardArray.length; i++) 
    {
      if(boardArray[i] === "-") 
      {
        var existingValues = getAllIntersections(boardArray, i);
        // We will choose the value that has not occured and if value has occured we Backtrack.
        var choices = ["1", "2", "3", "4", "5", "6", "7", "8", "9"].filter(function (num) {
          return existingValues.indexOf(num) < 0;
        });
        return { index: i, choices: choices }; // Js object
      }
    }
  }

  function getAllIntersections(boardArray, i) {
    return getRow(boardArray, i).concat(getColumn(boardArray, i)).concat(getBox(boardArray, i)); // adds all strings and returns a string
  }

  function allRowsValid(boardArray) {
    return [0, 9, 18, 27, 36, 45, 54, 63, 72].map(function (i) {  // starting indices of each row
  ...
  function getRow(boardArray, i) {  // slicing the string and returning the chunks of 9 elements each
  ...
  function allBoxesValid(boardArray) {
    return [0, 3, 6, 27, 30, 33, 54, 57, 60].map(function (i) {  // starting indices of all boxes
  ...
  function getBox(boardArray, i) {
    ...
    return [0, 1, 2, 9, 10, 11, 18, 19, 20].map(function (num) {  // adding the starting indicies of mini boxes to these will take us to each cell in every mini box.These are basically the indices of elements in the first box.
